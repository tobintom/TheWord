...
/**
 * Generate from word project
 * @author tobin
 *
 */
public class GenerateJSONFromFolder {
	...
	public static void main(String[] args) {
		try{
			...
			try (Writer writer = new OutputStreamWriter(
						    new FileOutputStream("X:\\EclipseWorkspace\\Datasetup\\dir\\FIJ.json"), "UTF-8");					
		             
		        ) {		            
				    
					displayDirectoryContents(new File("X:\\EclipseWorkspace\\Datasetup\\dir\\FIJ"),null,writer);
					writer.flush();            
		        }		
	...
	private static void displayDirectoryContents(File dir, String chap,Writer writer) {
		try {
			String book = chap;
			...
			for (File file : files) {
				if (file.isDirectory()) {
					...
					displayDirectoryContents(file,file.getName(),writer);
				} else {
					if(book!=null)
					printBook(book, file,writer);
	...
	private static void printBook(String book, File file,Writer writer){
		String chapter = "";
		try{
			...
			chapter = filename.substring(0, filename.indexOf("."));
			...
			for(Element e:elements){
				...
				for(String verse:verses){
		    		...
		    		compactJson.put("verse", verse.substring(verse.indexOf(">")+1,verse.indexOf("</span>")).trim());
			...
			System.err.println(book + " Chapter "+chapter );
			e.printStackTrace();
			System.exit(0);
		}
	}
